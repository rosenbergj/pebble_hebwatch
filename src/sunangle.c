...
double time_of_elevation(double jc, int tz, float longitude, float latitude, float elev_angle, int pm) {
    ...
    /* if |hacos| > 1, the arccos can't be computed (and the sun never reaches this angle today). */
    /* TODO: one "arctic" error variable isn't enough; we might be checking multiple different angles */
    double target_ha;
#ifdef CHECK_ARCTIC_ERROR
    if(CHECK_ARCTIC_ERROR && c_abs(hacos) > .98) {
        arctic = (hacos > 0 ? 1 : -1);
        target_ha = -1;
    } else {
        target_ha = 90 - (arcsin_cordic(hacos, 30)*180/C_PI);
    }
#else
        target_ha = 90 - (arcsin_cordic(hacos, 30)*180/C_PI);
#endif    
...
/*double elevation_angle(double jc, int tz, float longitude, float latitude) {
    ...
    return arcsin_cordic(elevsin, CORDIC_SIZE)*180/C_PI;
}
*/